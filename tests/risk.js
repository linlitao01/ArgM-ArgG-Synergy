/* Early-warning dashboard screenshot (risk-matrix chart) */
...
const out = process.argv[2] || "_risk.png";
const PORT = 9888;

const chrome = spawn(CHROME, ["--headless=new", "--disable-gpu", `--remote-debugging-port=${PORT}`, "--window-size=1500,1300", "about:blank"], { stdio: "ignore" });
...
(async () => {
  ...
  await sleep(7000);
  await evaljs(`document.querySelector('.tab[data-tab="warning"]').click(); true`);
  await sleep(500);
